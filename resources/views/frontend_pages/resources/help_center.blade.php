...
<main class="pt-[5.5rem] lg:pt-24">
    <!-- Page Title -->
    <section
      class="after:bg-jacarta-900/60 relative bg-cover bg-center bg-no-repeat py-20 after:absolute after:inset-0"
      style="background-image: url('{{asset("uploads/images/help_center")}}/{{ helpCenterTitle()->bg_image }}')"
    >
      <div class="container relative z-10">
        <h1 class="font-display mb-4 text-center text-4xl font-medium text-white">{{ helpCenterTitle()->help_center_title }}</h1>
        <form class="relative mx-auto block max-w-md">
          <input
            type="search"
            id="searchQuestion"
            class="text-jacarta-700 placeholder-jacarta-500 focus:ring-accent border-jacarta-100 w-full rounded-2xl border py-[0.6875rem] px-4 pl-10"
            placeholder="Search"
          />
          <input
            type="hidden"
            id="searchQuestionSubmitBtn"
          />
          <span class="absolute left-0 top-0 flex h-full w-12 items-center justify-center rounded-2xl">
            <svg
              ...
              width="24"
              height="24"
              class="fill-jacarta-500 h-4 w-4"
            >
    ...
    <div style="margin-top:100px">
        <h2 class="font-display text-jacarta-700 mb-10 text-center text-xl font-medium dark:text-white">
            {{ helpCenterTitle()->faq_title }}
        </h2>
        <div class="accordion mx-auto max-w-[35rem]" id="accordionFAQ">
            @include('frontend_pages.resources.faqSearch')
        </div>
        <p class="text-jacarta-300 mx-auto mb-10 max-w-md text-center text-lg">
            {{ helpCenterTitle()->faq_meta_title }}
          <a href="{{ route('contact') }}" class="text-accent">{{ __('Contact Support') }}</a>
        </p>
        </div>
    </div>
    <!-- Help Center -->
    <section class="relative py-24">
      ...
      <div class="container">
        <h2 class="font-display text-jacarta-700 mb-10 text-center text-xl font-medium dark:text-white">
            {{ helpCenterTitle()->category_title }}
        </h2>
        ...
        <div class="mb-16 grid grid-cols-1 gap-7 sm:grid-cols-2 md:grid-cols-3">
            ...
            <a
              class="dark:border-jacarta-600 dark:bg-jacarta-700 rounded-2lg border-jacarta-100 border bg-white p-6 text-center transition-shadow hover:shadow-lg"
            >
...
<script>
    $(document).ready(function () {
        $("body").on('input', '#searchQuestion', function (e) {
            let search_value = $('#searchQuestion').val();
            $.ajaxSetup({
                headers: {
            ...
            $.ajax({
                ...
                success: function (response){
                    $('#accordionFAQ').html(response.searchData);
                }
            });
        });
    });
