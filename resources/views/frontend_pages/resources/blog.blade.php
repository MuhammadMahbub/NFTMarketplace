...
<main class="pt-[5.5rem] lg:pt-24">
    ...
    <section class="relative py-16 md:py-24">
      ...
      <div class="container">
        ...
        <article class="mb-[1.875rem] md:mb-16">
          <div class="rounded-2.5xl flex flex-col overflow-hidden transition-shadow hover:shadow-lg md:flex-row">
            <figure class="group overflow-hidden md:w-1/2">
              <a href="{{ route('single_post', $blogs->first()->slug) }}">
                <img
                  src="{{asset('uploads/images/blog')}}/{{ $blogs->first()->main_image }}"
                  alt="post 1"
            ...
            <!-- Body -->
            <div
              class="dark:border-jacarta-600 dark:bg-jacarta-700 border-jacarta-100 rounded-b-[1.25rem] border bg-white p-[10%] md:w-1/2 md:rounded-none md:rounded-r-[1.25rem]"
            >
              <!-- Meta -->
              <div class="mb-3 flex flex-wrap items-center space-x-1 text-xs">
                <a href="#" class="dark:text-jacarta-200 text-jacarta-700 font-display hover:text-accent"
                  >{{ $blogs->first()->author_name }}</a
                >
                <span class="dark:text-jacarta-400">by</span>
                <span class="text-accent inline-flex flex-wrap items-center space-x-1">
                  <a href="#">{{ $blogs->first()->relationToBlogCategory->category_name ?? '' }}</a>
                </span>
              </div>

              <h2
                class="font-display text-jacarta-700 dark:hover:text-accent hover:text-accent mb-4 text-xl dark:text-white sm:text-3xl"
              >
                <a href="{{ route('single_post', $blogs->first()->slug) }}"> {{ $blogs->first()->title }} </a>
              </h2>
              <p class="dark:text-jacarta-200 mb-8">
                {{ $blogs->first()->subtitle }}
              </p>

              <!-- Date / Time -->
              <div class="text-jacarta-400 flex flex-wrap items-center space-x-2 text-sm">
                <span><time datetime="2022-02-05">{{ $blogs->first()->created_at->diffForHumans() }}</time></span>
              </div>
            </div>
          </div>
        </article>
        ...
        <div class="grid grid-cols-1 gap-[1.875rem] sm:grid-cols-2 md:grid-cols-3" id="blog_data">
          <!-- Posts -->
          @include('frontend_pages.resources.blog_load_more')
        </div>

        <!-- Load More -->
        <div class="load_more_button">
            <div class="mt-10 text-center">
              <a href="#!" id="load-more" data-count="3" class="load_more bg-accent shadow-accent-volume hover:bg-accent-dark inline-block rounded-full py-3 px-8 text-center font-semibold text-white transition-all">
                Load More
              </a>
            </div>
        </div>
  ...
  <!-- Buy Now Modal -->
  <div class="modal fade" id="buyNowModal" tabindex="-1" aria.0-labelledby="buyNowModalLabel" aria-hidden="true">
